...
export default function Home() {
  ...
  async function addComment(event: React.FormEvent) {
    event.preventDefault();
    if (!commentTask || !commentDraft.trim() || commentSubmitting) return;
    setCommentSubmitting(true);
    try {
      const response = await fetch('/api/comments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ taskId: commentTask.id, body: commentDraft.trim(), author: 'JD' }) });
